...
import {getData} from "@/components/house/HousePageAPI";
...
const House = (props: HouseProps) => {
  const [housePageDTO, setHousePageDTO] = useState<HousePageDTO>();
  const router = useRouter();

  const [open, setOpen] = useState(false)

  useEffect(() => {
    getData(props.id).then(setHousePageDTO)
  }, [])
                ...
                <section className={"flex gap-1 justify-between"}>
                  ...
                  <div className={"flex gap-1"}>
                    <div>
                      ...
                      {housePageDTO && housePageDTO.images &&
